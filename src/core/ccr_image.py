...
class CCRImage:
    ...
    def compute_namicolor_gain(self) -> float:
        ...
        crop = getattr(self, "crop_rect", None)
        angle = getattr(self, "crop_angle", 0.0) or 0.0
        # Auto-gain over the CROP area when set, else the whole image (the auto-
        # gain's own holder mask still ignores the brightest sprocket/holder
        # outliers). Frame auto-detection is intentionally NOT used here.
        meas = apply_crop_to_image(self.resized_raw, crop, angle) if crop else self.resized_raw
